...
#include "otfftpp/detail/otfft_avxdif4.h"
...
#else
                const zmm w1p = duppz5(conj(*twidT<8,N,1>(W,sp)));
                const zmm w2p = duppz5(conj(*twidT<8,N,2>(W,sp)));
                const zmm w3p = duppz5(conj(*twidT<8,N,3>(W,sp)));
                const zmm w4p = duppz5(conj(*twidT<8,N,4>(W,sp)));
                const zmm w5p = duppz5(conj(*twidT<8,N,5>(W,sp)));
                const zmm w6p = duppz5(conj(*twidT<8,N,6>(W,sp)));
                const zmm w7p = duppz5(conj(*twidT<8,N,7>(W,sp)));
#endif
               for (int q = 0; q < s; q += 4) {
                   complex_vector xq_sp  = x + q + sp;
                   complex_vector yq_s8p = y + q + s8p;
